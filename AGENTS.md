...
- **2026-08-20 — Hosting backend su Railway (piano free)**: aggiunti `backend/start.sh` (migrate + `php artisan serve` su `0.0.0.0:$PORT`) e `backend/railway.json` (build/start/healthcheck su `/up`) per farlo riconoscere da Railpack. Prerequisito non automatizzabile da repo: nella dashboard Railway il servizio deve avere **Root Directory = `backend`** (il repo è un monorepo, Railpack analizza altrimenti la root dove non trova `composer.json`/`artisan`). Dettagli completi in [docs/deployment.md](./docs/deployment.md).
