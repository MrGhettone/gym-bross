...
- **Service Worker**: strategia `injectManifest` (non più `generateSW` dalla Fase 7 — serve un service worker custom, `src/sw.ts`, per gestire gli eventi push/notificationclick). Precache dell'app shell delegato comunque a workbox (`precacheAndRoute(self.__WB_MANIFEST)` dentro `src/sw.ts`). `registerType: autoUpdate` — nessun prompt "nuova versione disponibile", si aggiorna in background. **Attenzione**: con `injectManifest`, a differenza di `generateSW`, vite-plugin-pwa non aggiunge automaticamente `self.skipWaiting()`/`clientsClaim()` al service worker — vanno chiamati esplicitamente in `src/sw.ts` (bug reale risolto il 2026-09-13, vedi [AGENTS.md](../AGENTS.md#decisioni-architetturali--problemi-aperti)), altrimenti un nuovo deploy resta "in attesa" finché l'utente non chiude tutte le schede dell'app.
